...
class TestApp(BaseTests):
    ...
    def test_not_existing_dataset(self,client: FlaskClient, faker: Faker,app: Flask,)-> None:
    ...
    def test_cache_clean_with_empty_cache_dir(self,client: FlaskClient, faker: Faker,app: Flask)-> None:
        # delete cache dir
        CACHE_DIR.rmdir()
        # check system error if cache dir does not exists
        with pytest.raises(Ignore, match=r"^(.*?(\bInvalid CACHE_PATH config\b)[^$]*)$"):
            self.send_task(
                app,
                "clean_cache"
            )
