...
if (typeof document === "undefined") {
  ...
  (function (global) {
    ...
    catiline._hasWorker =
      typeof Worker !== "undefined" && typeof fakeLegacy === "undefined";
          ...
          if (b) {
            "importScripts(" +
              b.split(",").forEach(function (cc) {
                matches[
                  catiline.makeUrl(cc.match(/\s*[\'\"](\S*)[\'\"]\s*/)[1])
                ] = true; // trim whitespace, add to matches
              }) +
              ");\n";
          }
        };
    ...
    catiline.makeIWorker = function (strings, codeword) {
      ...
      window.addEventListener("message", function (e) {
        if (
          ...
          e.data.length > codeword.length &&
          e.data.slice(0, codeword.length) === codeword
        ) {
    ...
    catiline.Worker = function Catiline(obj) {
      ...
      var __codeWord__ =
        "com.catilinejs." +
        (catiline._hasWorker ? "iframe" : "worker") +
        Math.random();
      var listeners = {};
      var self = this;
          ...
          if (listeners[eventName].indexOf(func) > -1) {
            if (listeners[eventName].length > 1) {
              ...
              listeners[eventName].splice(
                listeners[eventName].indexOf(func),
                1
              );
            }
          }
